/* A project to monitor water quality in remote locations
  ...
  sensors are not implemented in Spudnik-09.
  ...
  //   see: https://community.particle.io/t/recover-electron-from-beaver-attack/
       {
        minutes = 60;  // 7 hours (420 min)
         if (fuel.getSoC() >25)   minutes = 30;    // 5 hours (300 min)
            if (fuel.getSoC() >40)   minutes = 20;     // 2 hours (120 min)
                if (fuel.getSoC() >60)   minutes = 10;   // 1 hours (60 min)
                    if (fuel.getSoC() >70)   minutes = 10;    // 45 minutes
                        if (fuel.getSoC() >75)   minutes = 10;     // 30 minutes
